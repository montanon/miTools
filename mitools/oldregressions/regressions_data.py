...
from mitools.utils import add_significance, replace_prefix
    ...
    def to_pretty_df(self):
        ...
        try:
            regression_data["Result"] = (
                regression_data[self.coefficient_col].round(3).astype(str)
                + " ("
                + regression_data[self.significance_col].astype(str)
                + ")"
            )
            regression_data["Result"] = regression_data["Result"].apply(
                add_significance
            )
        except Exception:
            regression_data["Result"] = ""
...
class OLSResults(RegressionData):
    ...
    def to_pretty_df(self):
        regression_data = self.to_df()
        regression_data["Result"] = (
            regression_data[self.coefficient_col].round(2).astype(str)
            + " ("
            + regression_data[self.significance_col].astype(str)
            + ")"
        )
...
class CSARDLResults(RegressionData):
    ...
    def __post__init__(self):
        assert all(
            replace_prefix(v, "L.", "") in self.short_run_coefficients.keys()
            for v in self.indep_variables
        )
        assert all(
            replace_prefix(v, "L.", "") in self.short_run_std_errs.keys()
            for v in self.indep_variables
        )
        assert all(
            replace_prefix(v, "L.", "") in self.short_run_z_values.keys()
            for v in self.indep_variables
        )
        assert all(
            replace_prefix(v, "L.", "") in self.short_run_p_values.keys()
            for v in self.indep_variables
        )
        assert all(
            replace_prefix(v, "L.", "") in self.short_run_significances.keys()
            for v in self.indep_variables
        )
        assert all(
            replace_prefix(v, "L.", "") in self.short_run_conf_intervals.keys()
            for v in self.indep_variables
        )

        assert all(
            replace_prefix(v, "lr_", "") in self.long_run_coefficients.keys()
            for v in self.indep_variables
        )
        assert all(
            replace_prefix(v, "lr_", "") in self.long_run_std_errs.keys()
            for v in self.indep_variables
        )
        assert all(
            replace_prefix(v, "lr_", "") in self.long_run_z_values.keys()
            for v in self.indep_variables
        )
        assert all(
            replace_prefix(v, "lr_", "") in self.long_run_p_values.keys()
            for v in self.indep_variables
        )
        assert all(
            replace_prefix(v, "L.", "") in self.long_run_significances.keys()
            for v in self.indep_variables
        )
        assert all(
            replace_prefix(v, "lr_", "") in self.long_run_conf_intervals.keys()
            for v in self.indep_variables
        )

        assert all(
            replace_prefix(v, "lr_", "") in self.adj_term_coefficients.keys()
            for v in self.indep_variables
        )
        assert all(
            replace_prefix(v, "lr_", "") in self.adj_term_std_errs.keys()
            for v in self.indep_variables
        )
        assert all(
            replace_prefix(v, "lr_", "") in self.adj_term_z_values.keys()
            for v in self.indep_variables
        )
        assert all(
            replace_prefix(v, "lr_", "") in self.adj_term_p_values.keys()
            for v in self.indep_variables
        )
        assert all(
            replace_prefix(v, "L.", "") in self.adj_term_significances.keys()
            for v in self.indep_variables
        )
        assert all(
            replace_prefix(v, "lr_", "") in self.adj_term_conf_intervals.keys()
            for v in self.indep_variables
        )
    ...
    def to_df(self):
        ...
        df.index = MultiIndex.from_product(
            [
                list([self.dep_variable]),
                [self.model_params.get("lag", None)],
                df["type"].values,
            ]
        )

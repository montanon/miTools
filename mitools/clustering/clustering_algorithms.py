...
def plot_kmeans_ncluster_search(
    silhouette_scores: List[float], inertia: List[float]
) -> Axes:
    ...
    for x, y in zip(x_values_silhouette, silhouette_scores):
        ax[0].vlines(
            x, min_y_silhouette, y, linestyles="dotted", colors="grey", linewidth=0.5
        )
    ...
    for x, y in zip(x_values_inertia, inertia):
        ax[1].vlines(
            x, min_y_inertia, y, linestyles="dotted", colors="grey", linewidth=0.5
        )

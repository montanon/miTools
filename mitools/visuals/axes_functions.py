...
def adjust_axes_lims(axes: Axes, mode: Optional[str]='all', x: Optional[bool]=True, y: Optional[bool]=True) -> Axes:
    if not (x or y):
        return axes
    ...
    if mode == 'all':
        ...
        raise ValueError(f'Unknown mode: {mode}, must be one of "all", "rows", "columns"')
    return axes
...
def set_axes_limits(axes: Axes, set_lim_func: Callable, lim_min: float, lim_max: float) -> None:
    for ax in axes:

...
def create_language_info(codemirror_mode: CodeMirrorMode, file_extension: str, mimetype: str, 
                         name: str, nbconvert_exporter: str, pygments_lexer: str, version: str
                         ) -> LanguageInfo:
    return LanguageInfo(
...
def create_notebook_metadata(language_info: LanguageInfo, kernelspec: Optional[KernelSpec]=None
                             ) -> NotebookMetadata:
...
def create_notebook_cell(cell_type: str, execution_count: None, cell_id: str, metadata: Dict, 
                         outputs: List, source: List) -> NotebookCell:
    cell = NotebookCellFactory.create_cell(cell_type=cell_type,
...
def create_notebook(cells: List[NotebookCell], metadata: NotebookMetadata, nbformat: int, 
                    nbformat_minor: int, name: Optional[str]='', notebook_id: Optional[str]=''
                    ) -> Notebook:     
    return Notebook(
        cells=cells,
        metadata=metadata,

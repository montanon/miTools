...
SPLIT_PATTERN = """===============================================================================\n> ==========\n"""
REGRESSION_PATTERN = (
    f"({SPLIT_PATTERN}){{2}}(.*?)({SPLIT_PATTERN}){{2}}"  # (.*?)({SPLIT_PATTERN}){{1}}'
)
...
def threaded_process_logs(
    logs_paths, model_types, batch_size: Optional[int] = 1, n_threads: Optional[int] = 1
):
    if n_threads > 1:
...
def get_xtreg_fe_data_from_log(xtreg_str: str):
    n_obs = get_numbers_from_str(
        re.search(rf"Number of obs += +\n* *-?\d*\,*\d*\.*\d+\n", xtreg_str).group(0)
    )[-1]
    n_groups = get_numbers_from_str(
        re.search(
            rf"Number of groups += +\n*(({NUMBER_PATTERN})|(.))+\n", xtreg_str
        ).group(0)
    )[-1]
    try:
        n_obs_per_group = get_numbers_from_str(
            re.search(r"Obs per group \(T\) += +-?\d*\.*\d+\n", xtreg_str).group(0)
        )[-1]
    except AttributeError:
        n_obs_per_group = get_numbers_from_str(
            re.search(rf"avg *= +(({NUMBER_PATTERN})|(.))+\n", xtreg_str).group(0)
        )[-1]
    F_stats = get_numbers_from_str(
        re.search(
            rf"F\(-?\d+, -?\d+\) += +(({NUMBER_PATTERN})|(.))+\n", xtreg_str
        ).group(0)
    )[-1]
    Prob_F = get_numbers_from_str(
        re.search(rf"Prob > F += +(({NUMBER_PATTERN})|(.))+\n", xtreg_str).group(0)
    )[-1]
    R_sq = get_numbers_from_str(
        re.search(rf"Overall += +(({NUMBER_PATTERN})|(.))+\n", xtreg_str).group(0)
    )[-1]
    R_sq_within = get_numbers_from_str(
        re.search(rf"Within += +(({NUMBER_PATTERN})|(.))+\n", xtreg_str).group(0)
    )[-1]
    R_sq_between = get_numbers_from_str(
        re.search(rf"Between += +(({NUMBER_PATTERN})|(.))+\n", xtreg_str).group(0)
    )[-1]
    corr = get_numbers_from_str(
        re.search(rf"corr\((.*?)= +(({NUMBER_PATTERN})|(.))+\n", xtreg_str).group(0)
    )[-1]
    ...
    dep_variable = (
        re.search("(Indicat(?:~\d+X|or\w+X))|(ECI)", coefficients_table)
        .group(0)
        .strip()
    )

    coefficients_pattern = re.compile(
        r"(^\s*\w+\s*)|(^Indi[a-zX~0-9]*)\|.*[-+]?[0-9]*\.?[0-9]+(e[-+]?[0-9]+)?|(\s*\.\s*)+$"
    )
    coefficient_rows = [
        line
        for line in coefficients_table.split("--------+")[1].split("\n")
...
def get_ols_data_from_log(ols_str: str):
    ...
    F_stats = get_numbers_from_str(
        re.search(r"F\(\d+, \d+\) += +-?\d*\.*\d+\n", ols_str).group(0)
    )[-1]
    Prob_F = get_numbers_from_str(
        re.search(r"Prob > F += +-?\d*\.*\d+\n", ols_str).group(0)
    )[-1]
    R_sq = get_numbers_from_str(
        re.search(r"R-squared += +-?\d*\.*\d+\n", ols_str).group(0)
    )[-1]
    AdjR_sq = get_numbers_from_str(
        re.search(r"Adj R-squared += +-?\d*\.*\d+\n", ols_str).group(0)
    )[-1]
    Root_MSE = get_numbers_from_str(
        re.search(r"Root MSE += *\n*-?\d*\.*\d*([eE][\+\-]\d+)?", ols_str).group(0)
    )[-1]
    coefficients_table = ols_str.split("\n\n")[1]

    dep_variable = (
        re.search("Indicat(?:~\d+X|or\w+X)", coefficients_table).group(0).strip()
    )
...
def get_csardl_data_from_log(csardl_str):
    ...
    R_sq = get_numbers_from_str(
        re.search(rf"R-squared += +(({NUMBER_PATTERN})|(.))+\n", csardl_str).group(0)
    )[-1]
    R_sq_MG = get_numbers_from_str(
        re.search(
            rf"R-squared \(MG\) += +(({NUMBER_PATTERN})|(.))+\n", csardl_str
        ).group(0)
    )[-1]
    Root_MSE = get_numbers_from_str(
        re.search(rf"Root MSE += +(({NUMBER_PATTERN})|(.))+\n", csardl_str).group(0)
    )[-1]
    try:
        CD_stats = get_numbers_from_str(
            re.search(
                rf"CD Statistic += +(({NUMBER_PATTERN})|(.))+\n", csardl_str
            ).group(0)
        )[-1]
    except AttributeError as e:
        CD_stats = 0
    try:
        p_value = get_numbers_from_str(
            re.search(rf"p-value += +(({NUMBER_PATTERN})|(.))+\n", csardl_str).group(0)
        )[-1]
    except AttributeError as e:
        p_value = 0
    ...
    end_table_line = find_str_line_number_in_text(
        coefficients_table, "Mean Group Variables:"
    )

    short_run_coefficients = coefficients_table_lines[
        short_run_line + 3 : adj_term_line - 1
    ]
    adj_term_coefficients = coefficients_table_lines[
        adj_term_line + 3 : long_run_line - 1
    ]
    long_run_coefficients = coefficients_table_lines[
        long_run_line + 3 : end_table_line - 1
    ]

    short_run_coefficients = get_coefficients_from_table_rows(
        short_run_coefficients, var_names
    )
    adj_term_coefficients = get_coefficients_from_table_rows(
        adj_term_coefficients, var_names
    )
    long_run_coefficients = get_coefficients_from_table_rows(
        long_run_coefficients, var_names
    )
    ...
    try:
        model_specification = re.search(
            "capture noisily: ", csardl_str, re.DOTALL
        ).group(0)
...
def generate_significance_color_styles(df):
    ...
    for r in range(df.shape[0]):
        for c in range(df.shape[1]):
            ...
            if (
                isinstance(val, str)
                and "ECI" in df.index[r][-1]
                or "SCP" in df.index[r][-1]
                or "SCI" in df.index[r][-1]
                or "PSEV" in df.index[r][-1]
            ):
                ...
                if "***" in val:
                    val_style = (
                        ...
                        if pos_value
                        else "background-color: red; font-weight: bold;"
                    )
                elif "**" in val:
                    val_style = (
                        "background-color: springgreen; font-weight: bold;"
                        if pos_value
                        else "background-color: orangered; font-weight: bold;"
                    )
                elif "*" in val:
                    val_style = (
...
def df_view(
    df, indicators, columns, col_filters, index_filters, eci_col, col_name, row_name
):
...
def indep_var_sorting_key(val):
    ...
    return pd.Index(
        [string_value[v] if v in string_value else 99 for v in val], name="Indep Var"
    )
...
def style_csardl_results(selection, row_level_name, col_level_name):
    ...
    return selection.style.apply(
        lambda x: color_style + cell_border_style, axis=None
    ).set_table_styles(indexes_style)
...
def mask_results(dataframe, indicator_names):
    ...
    dataframe["Variable"] = dataframe["Variable"].apply(
        lambda x: ind_var_name_replace(x, indicator_names)
    )
    dataframe["Dep Var"] = dataframe["Dep Var"].apply(
        lambda x: ind_var_name_replace(x, indicator_names)
    )
    dataframe["Indep Var"] = dataframe["Indep Var"].map(
        indicator_names.to_dict()["Original Name"]
    )
    dataframe["Variable"] = dataframe["Variable"].apply(
        lambda x: ind_var_name_replace(x, indicator_names)
    )
...
def ind_var_name_replace(string, indicator_names):
    ...
    if search_indicator:
        ...
        if (
            indicator not in indicators_dict.values()
            and indicator in indicators_dict.keys()
        ):
            ...
            raise Exception
        string = re.sub(
            "(?<=[._])?[A-Za-z& ]*(ASCI|SECI|ECI|SCI|SCP|PSEV)", indicator_name, string
        )
        if string.find("~") > -1:
            print(string)
    return string
...
def generate_hash_from_dataframe(
    df, index_levels: List[str], length: Optional[int] = 6
):
    str_to_hash = ""

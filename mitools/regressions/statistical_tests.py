...
def shapiro_test_dataframe_groups(
    data: Dict[str, DataFrame], criteria: Optional[float] = 0.05
):
    shapiro_tests = []
    for group, group_data in data.items():
        statistics = group_data.apply(shapiro_test, axis=0, result_type="expand")
        statistics = statistics.T
        statistics["hypothesis"] = statistics["p-value"].apply(
            lambda x: "Reject" if x < criteria else "Accept"
        )
        statistics = statistics.T
...
def anderson_test_dataframe_groups(
    data: Dict[str, DataFrame], criteria: Optional[float] = 0.01
):
    anderson_tests = []
    for group, group_data in data.items():
        statistics = group_data.apply(
            anderson_test, args=(criteria,), axis=0, result_type="expand"
        )
        statistics = statistics.T
        statistics["hypothesis"] = statistics.apply(
            lambda x: "Reject" if x["statistic"] > x["critical_value"] else "Accept",
            axis=1,
        )
        statistics = statistics.T
        statistics.index.name = "statistics"
...
def adf_test_dataframe_groups(
    ...
    for group, group_data in data.items():
        ...
        statistics["hypothesis"] = statistics.apply(
            lambda row: "Reject"
            if row["statistic"] < row[f"critical_value_{critical_value}%"]
            else "Accept",
            axis=1,
        )
        statistics = statistics.T
...
def calculate_vif_dataframe_groups(
    data: Dict[str, DataFrame], dependent_var: str, threshold: Optional[float] = 5
):
    vif_scores = []
    for group, group_data in data.items():
...
def calculate_dw(data, dependent_var):
    ...
    return pd.DataFrame(
        {"DW Statistic": dw_stat, "Hypothesis": hypothesis}, index=[dependent_var]
    )
...
def calculate_white_test_dataframe_groups(
    data: Dict[str, DataFrame], dependent_var: str
):
    white_tests = []
    for group, group_data in data.items():

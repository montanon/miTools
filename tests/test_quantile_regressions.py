    ...
    def test_constants_presence_type_and_exclusivity(self):
        ...
        expected_constants = set(
            [
                ...
                "QUADRATIC_REG",
                "LINEAR_REG",
                "QUADRATIC_VAR_SUFFIX",
                "INDEPENDENT_VARS_PATTERN",
                "STATS",
                "INTERCEPT",
                "ANNOTATION",
                "PARQUET_SUFFIX",
        ...
        actual_constants = set(
            [attr for attr in dir(QuantileRegStrs) if not attr.startswith("__")]
        )
        ...
        self.assertFalse(
            expected_constants - actual_constants,
...
class TestCreateRegressionFilePaths(unittest.TestCase):
    ...
    def test_file_paths_creation(self):
        eci_type_folder = Path(
            "/tmp/eci_type_folder"
        )  # Use a temporary directory for testing
        regression_id = "123"
        main_plot, regression_plot = create_regression_file_paths(
            eci_type_folder, regression_id
        )
    ...
    def test_input_types(self):
        eci_type_folder_str = (
            "/tmp/eci_type_folder"  # Use a temporary directory for testing
        )
        eci_type_folder_path = Path(eci_type_folder_str)
        regression_id = "123"
        main_plot_path, regression_plot_path = create_regression_file_paths(
            eci_type_folder_path, regression_id
        )
        main_plot_str, regression_plot_str = create_regression_file_paths(
            eci_type_folder_str, regression_id
        )
        # Add the created file paths to the list for cleanup
        self.created_files.extend(
            [main_plot_path, regression_plot_path, main_plot_str, regression_plot_str]
        )
...
class TestGetGroupData(unittest.TestCase):
    ...
    def test_filtering_by_group(self):
        ...
        self.assertTrue(
            (result.index == "A").all()
        )  # All rows should belong to group 'A'
...
class TestPrepareXValues(unittest.TestCase):
        ...
        self.group_data = DataFrame(
            {
                ...
                "x3" + QuantileRegStrs.QUADRATIC_VAR_SUFFIX: np.random.rand(
                    10
                ),  # Quadratic variable
            }
        )
    ...
    def test_correct_columns(self):
        ...
        expected_columns = [
            "x1",
            "x2",
        ]  # 'x3' with the quadratic suffix should be excluded
    ...
    def test_linear_spacing(self):
        ...
        for var in independent_vars:
            ...
            self.assertTrue(
                np.allclose(np.diff(generated_values), np.diff(generated_values)[0]),
                f"Column {var} values are not linearly spaced",
            )
...
class TestGetPrediction(unittest.TestCase):
    ...
    def test_linear_prediction(self):
        ...
        coeffs = {
            self.strs.LINEAR_REG: 2,
            self.strs.INTERCEPT: 5,
        }  # coeffs[0] is the slope, coeffs[-1] is the intercept
        quadratic = False
        expected_predictions = (
            coeffs[self.strs.INTERCEPT] + coeffs[self.strs.LINEAR_REG] * x_values
        )
    ...
    def test_quadratic_prediction(self):
        ...
        coeffs = {
            self.strs.LINEAR_REG: 2,
            self.strs.QUADRATIC_REG: 4,
            self.strs.INTERCEPT: 5,
        }  # coeffs[1] is the quadratic term coefficient, coeffs[-1] is the intercept
        quadratic = True
        expected_predictions = (
            coeffs[self.strs.INTERCEPT]
            + coeffs[self.strs.LINEAR_REG] * x_values
            + coeffs[self.strs.QUADRATIC_REG] * x_values**2
        )
        actual_predictions = x_values.apply(get_prediction, args=(coeffs, quadratic))
        pd_testing.assert_series_equal(actual_predictions, expected_predictions)
    ...
    def test_empty_series(self):
        ...
        actual_predictions = x_values.apply(get_prediction, args=(coeffs, quadratic))
        self.assertTrue(
            actual_predictions.empty,
            "Result should be an empty Series when input Series is empty",
        )
